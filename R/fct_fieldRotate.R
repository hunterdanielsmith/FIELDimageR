...
#' 
#' 
#' @importFrom terra rast crs plotRGB res project ext atan2 as.list as.data.frame as.matrix
#' @importFrom terra crop as.polygons mask extract diff
...
fieldRotate <- function(mosaic, theta = NULL, clockwise = TRUE, h = FALSE, n.core = NULL, extentGIS = FALSE, 
  ...
  if(!is.null(DSMmosaic)){
    par(mfrow=c(1,3))
    if(terra::crs(DSMmosaic)!=terra::crs(mosaic)){stop("DSMmosaic and RGBmosaic must have the same projection CRS")}}
  ...
  rotate <- function(x, angle=0, resolution=res(x)) {
    y <- x
    terra::crs(y) <- "+proj=aeqd +ellps=sphere +lat_0=90 +lon_0=0"
    project(y, res=resolution, paste0("+proj=aeqd +ellps=sphere +lat_0=90 +lon_0=", -angle))}
  if(is.null(theta)){
  ...
  if (!is.null(n.core)) {
    ...
    r <- foreach(i=nlyr(mosaic), .packages = c("terra")) %dopar% {rotate(mosaic[[i]], angle = theta)}
    parallel::stopCluster(cl)
  }
  if(extentGIS){
    m11<-apply(matrix(terra::ext(mosaic),2,byrow = T),1,function(x){mean(x)})
    m22<-apply(matrix(terra::ext(r),2,byrow = T),1,function(x){abs(diff(c(x[2],x[1]))/2)})
    terra::ext(r)<-c(as.numeric(c(m11[1]-m22[1])), as.numeric(c(m11[1]+m22[1])), as.numeric(c(m11[2]-m22[2])), as.numeric(c(m11[2]+m22[2])))
    terra::crs(r)<-terra::crs(mosaic)
  }
  Out<-r
  if(plot){
    if(fast.plot){
    ...
    if(!fast.plot){
      ...
      if(num.band<3){terra::plot(r, axes=FALSE, box=FALSE)}}}
  if(!is.null(DSMmosaic)){
    DSMmosaic <- terra::c(DSMmosaic)
    DSMmosaic <- rotate(DSMmosaic,angle = theta)
    terra::plot(DSMmosaic, axes=FALSE, box=FALSE)
    if(extentGIS){
      terra::ext(DSMmosaic)<-c(as.numeric(c(m11[1]-m22[1])), as.numeric(c(m11[1]+m22[1])), as.numeric(c(m11[2]-m22[2])), as.numeric(c(m11[2]+m22[2])))
      terra::crs(DSMmosaic)<-terra::crs(mosaic)
    }
