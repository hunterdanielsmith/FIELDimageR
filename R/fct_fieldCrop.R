...
#' 
#' @importFrom terra plotRGB mask nlyr scale
...
fieldCrop <- function(mosaic, fieldShape = NULL, nPoint = 4, plot = TRUE, remove = FALSE, type = "l", 
  ...
  if(!is.null(fieldShape)){
    if(terra::crs(fieldShape) != terra::crs(mosaic)){
      stop("fieldShape and mosaic must have the same terra::crs CRS. Use fieldRotate() for both files.")
      }
  ...
  if(plot){
    ...
    if(!fast.plot){
      if(num.band>2){plotRGB(r, r = 1, g = 2, b = 3)}
