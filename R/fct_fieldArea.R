...
#' 
#' @importFrom terra extract crop values
...
fieldArea <- function(mosaic, areaValue = 0, fieldShape = NULL, buffer = NULL, n.core = NULL, plot = TRUE, na.rm = FALSE) {
  ...
  if(plot){
    par(mfrow=c(1,1))
    terra::plot(mosaic, col=grey(1:100/100), axes=FALSE, box=FALSE)}
  if(!is.null(fieldShape)){
    ...
    if (!is.null(n.core)) {
      ...
      extM <- foreach(i = 1:length(fieldShape), .packages = c("terra")) %dopar% 
        {
          single <- fieldShape[i, ]
    ...
    values(fieldShape)<-cbind.data.frame(values(fieldShape),porarea)
