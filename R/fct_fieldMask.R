...
#' 
#' @importFrom utils read.csv
#' @importFrom terra plot project nlyr plotRGB c
#' 
...
fieldMask <- function(mosaic, Red = 1, Green = 2, Blue = 3, RedEdge = NULL, NIR = NULL, mask = NULL, index = "HUE",
  ...
  if(!is.null(mask)){
    ...
    if(projection){
      mosaic <- project(mosaic, mr, method = "ngb")
    }
  }
  ...
  if(!is.null(DSMmosaic)){
    par(mfrow=c(1,4))
    if(crs(DSMmosaic)!=crs(mosaic)){stop("DSMmosaic and RGBmosaic must have the same projection CRS, use fieldRotate() in both files.")}}
  
  if(plot){if(num.band>2){plotRGB(mosaic, r = 1, g = 2, b = 3)}
  ...
  if(!is.null(DSMmosaic)){
    ...
    if(projection){
      DSMmosaic <- project(DSMmosaic,mDEM,method = 'ngb')
    }
    DSMmosaic <- mask(DSMmosaic, mDEM, maskvalue=TRUE)
    if(plot){terra::plot(DSMmosaic, axes=FALSE, box=FALSE)}
    Out<-c(mosaic,m,DSMmosaic)
    names(Out)<-c(names(mosaic),"mask","DSMmosaic")
  }
  par(mfrow=c(1,1))
  return(Out)
}
